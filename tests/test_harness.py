...
class HarnessTests(unittest.IsolatedAsyncioTestCase):
    def test_create_task_generates_artifact_and_prompt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('id="harness-render-assets"', html)
    ...
    def test_artifact_runtime_v2_is_rendered_not_persisted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("renderMermaidAssets", rendered)
            self.assertIn('id="harness-render-assets"', persisted)
            self.assertIn('[data-render="mermaid"][data-diagram-id]', rendered)
            ...
            self.assertNotIn("harness-mermaid-rendered", saved)
            self.assertIn('id="harness-comments"', saved)
            self.assertIn('id="harness-render-assets"', saved)
...
class HarnessResetAndExportTests(unittest.TestCase):
    ...
    def test_export_learning_writes_markdown_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            expected_export_path = app.paths.learning_export_path(task["id"])
            prompt = app.prompt_builder.build(task)

            self.assertEqual(export_path, expected_export_path)
            self.assertIn(f"经验导出路径：{expected_export_path}", prompt)
